...
def minifyfiles(context):
    ...
    dst.text = (
        src.text.replace("${buildDate}", timeUTC()).replace(
    ...
    (minified - ".min.js" + ".amd.min.js").text = (
        """;(function(){
%s
...
def getCommitIDstring():
    ...
    if not hasattr(subprocess, "check_output"):
        ...
        return (
            "commit ID " + subprocess.check_output(["git", "rev-parse", "HEAD"]).strip()
        )
...
def compress_with_closure_compiler(code, compression_level=None):
    ...
    if response.status != 200:
        raise Exception(
            "Compilation server responded with non-OK status of " + str(response.status)
        )
    ...
    if "errors" in parts:
        ...
        for error in parts["errors"]:
            prettyerrors.append(
                "\nln %s, ch %s, '%s' - %s"
                % (error["lineno"], error["charno"], error["line"], error["error"])
            )
...
if __name__ == "__main__":
    print(
        "This is a Wak build automation tool script. Please, get Wak on GitHub and run it against the folder containing this automation script."
    )
