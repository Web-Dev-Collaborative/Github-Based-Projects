...
def couchid_to_authproxy_url(filename, couch_box):
    ...
    return url_for(
        "authproxy.couchdb_proxy_file", url=couch_box + "/" + urllib.quote(filename)
    )
...
def overview():
    if current_user.is_consultant:
        users = User.query.filter(
            or_(
                User.type == User.Type.CUSTOMER, User.type == User.Type.SERVICE_PROVIDER
            )
        )
