...
def geocouch_feature_to_geojson(feature):
    ...
    feature.pop("layer")
    result = {"type": "Feature", "geometry": geometry, "properties": feature}
    return result
...
class CouchDB(object):
    ...
    def put(self, doc_id, doc):
        ...
        resp = self.session.put(
            doc_url, headers={"Accept": "application/json"}, data=json.dumps(doc)
        )
    ...
    def update_user(self, user_id, password):
        user_doc = {"name": user_id, "password": password, "type": "user", "roles": []}
        resp = self.session.get(self.url + "/_users/org.couchdb.user:" + user_id)
        if resp.status_code == 200:
            user_doc["_rev"] = resp.json()["_rev"]
        resp = self.session.put(
            self.url + "/_users/org.couchdb.user:" + user_id,
    ...
    def update_auth_doc(self, user, writable=True, read_roles=[], write_roles=[]):
        auth_doc = {"_id": "_design/auth", "language": "javascript"}
        ...
        resp = self.session.put(
            self.couchdb_url + "/_security",
            data=json.dumps(
                {"members": {"names": [user], "roles": read_roles + ["_admin"]}}
            ),
            headers=(("Content-type", "application/json"),),
        )
...
class CouchDBBox(CouchDB):
    ...
    def layer_url(self, layer, include_docs=False):
        url = (
            self.couchdb_url
            + "/_design/layers/_view/all?key="
            + urllib.quote('"%s"' % layer.encode("utf8"))
        )
        if include_docs:
    ...
    def store_features(
        self, layer, features, update_unmodified=False, delete_missing=None
    ):
        """
    ...
    def clear_layer(self, layer):
        ...
        if resp.status_code != 200:
            raise CouchDBError(
                "could not read layer from %s: %s" % (resp.url, resp.content)
            )
        data = resp.json()
                ...
                files.append(
                    {
                        ...
                        "content_type": row["doc"]["_attachments"]["file"][
                            "content_type"
                        ],
                    }
                )
...
def init_user_boxes(user, couchdb_url):
    ...
    customer_couch = CouchFileBox(
        couchdb_url, "%s_%s" % (SystemConfig.CUSTOMER_BOX_NAME, user.id)
    )
    customer_couch.create()
    customer_couch.update_auth_doc(username, writable=True, read_roles=[])

    consultant_couch = CouchFileBox(
        couchdb_url, "%s_%s" % (SystemConfig.CONSULTANT_BOX_NAME, user.id)
    )
    consultant_couch.create()
    consultant_couch.update_auth_doc(
        username,
        writable=False,
        read_roles=["consultants"],
        write_roles=["consultants"],
    )
