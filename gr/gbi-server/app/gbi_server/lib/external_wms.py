...
class MapProxyConfiguration(object):
    ...
    def _load_sources(self):
        ...
        for wmts, view_coverage in public_wmts:
            self.sources["%s_source" % wmts.name] = {
                ...
                "url": wmts.url
                + wmts.layer
                + "/GoogleMapsCompatible-%(z)s-%(x)s-%(y)s/tile",
    ...
    def _write_mapproxy_yaml(self):
        ...
        services = {
            "demo": None,
            "wms": {"srs": self.service_srs, "md": {"title": _("external_wms_title")}},
        }
...
def write_mapproxy_config(app):
    mpc = MapProxyConfiguration(
        app.config.get("MAPPROXY_SRS"), app.config.get("MAPPROXY_YAML_DIR")
    )
    mpc._load_sources()
