...
class EditAddressForm(Form):
    ...
    zipcode = TextField(
        _l("zipcode"), [validators.Required(), validators.Regexp("[0-9]+$")]
    )
...
class NewUserForm(EditAddressForm):
    ...
    email = TextField(
        _l("email"), [validators.Required(), username_unique, validators.Email()]
    )
...
class RecoverRequestForm(Form):
    email = TextField(
        _l("email"), [validators.Required(), username_exists, validators.Email()]
    )
