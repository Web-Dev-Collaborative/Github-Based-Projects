...
class WMTSForm(Form):
    ...
    name = TextField(
        _l("wmts_name"), [validators.Required(), validators.Regexp("[a-zA-Z0-9_-]+$")]
    )
    ...
    matrix_set = TextField(
        _l("wmts_matrix_set"), [validators.Required()], default="GoogleMapsCompatible"
    )

    view_coverage = TextAreaField(
        _l("wmts_view_coverage"), [validators.Required()]
    )  # XXX kai: geojson validator?
    view_level_start = SelectField(
        _l("wmts_view_level_start"), coerce=int, choices=[(x, x) for x in range(21)]
    )
    view_level_end = SelectField(
        _l("wmts_view_level_end"), coerce=int, choices=[(x, x) for x in range(21)]
    )
    ...
    def validate_view_level_end(form, field):
        if form.data["view_level_start"] > field.data:
            raise validators.ValidationError(
                _l("level needs to be bigger or equal to start level")
            )
