...
from geobox.lib.coverage import (
    coverage_from_feature_collection,
...
def import_edit(id=None):
    ...
    base_layer = (
        g.db.query(model.ExternalWMTSSource).filter_by(background_layer=True).first()
    )
...
def export_edit(id=None):
    ...
    raster_sources = (
        g.db.query(model.LocalWMTSSource).order_by(model.LocalWMTSSource.id).all()
    )
    coverage_form = forms.SelectCoverage()
    ...
    form.srs.choices = [
        (srs, srs)
        for srs in current_app.config.geobox_state.config.get("web", "available_srs")
    ]
    if form.validate_on_submit():
        proj.title = form.data["title"]
        ...
        if form.data["mapping_name"] != "None":
            g.db.add(
                model.ExportVectorLayer(
                    mapping_name=form.data["mapping_name"], project=proj
                )
            )
        g.db.commit()
...
def create_raster_import_task(proj):
    ...
    local_raster_source = (
        g.db.query(model.LocalWMTSSource).filter_by(wmts_source=raster_source).first()
    )
