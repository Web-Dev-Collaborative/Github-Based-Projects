...
def tray_icon_thread(app_state):
    ...
    return TrayIconThread(
        app_state, host="localhost", port=app_state.config.get_int("web", "port")
    )
...
def mapproxy_thread(app_state):
    ...
    return WebServerThread(
        host, mapproxy_port, app, logger_name="geobox.mapproxy.server"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default=None, help="GeoBox client configuration.")
    parser.add_argument(
        "--open-webbrowser",
        default=False,
        action="store_true",
