...
class MapProxyConfiguration(object):
    ...
    def _load_sources(self):
        ...
        for local_source in local_sources:
            ...
            self.sources[wmts_source.name + "_source"] = {
                "type": "wms",
                "req": {"url": "http://dummy.example.org/service?"},
                "seed_only": True,
                "coverage": {
                    "srs": "EPSG:3857",
                    "bbox": list(
                        coverage_from_geojson(wmts_source.download_coverage).bbox
                    ),
                },
            }
            ...
            self.layers.append(
                {
                    ...
                    "max_res": self.grid.resolution(local_source.download_level_end)
                    / 2,
                }
            )
    ...
    def _write_mapproxy_yaml(self):
        ...
        services = {
            "demo": None,
            "wmts": None,
            "wms": {"srs": self.service_srs, "md": {"title": "Geobox WMS"}},
        }

        globals_ = {
            "image": {"paletted": True},
            "cache": {"meta_size": [8, 8], "meta_buffer": 50},
