...
def create_target_image(
    x_size, y_size, bbox, res, bands=3, band_type=1, t_name="out.tif", t_format="GTiff"
):
...
def _merge_tiles(
    ...
    gdal.FileFromMemBuffer(
        input_mem_buffer, convert_to_bands(first_tile["data"], mode).read()
    )

    if gdal_file_info.init_from_name(input_mem_buffer) == 1:  # add else for error
        dest = create_target_image(
            size[0],
            size[1],
            bbox,
            resolution,
        ...
        for tile in tile_iter:
            gdal.FileFromMemBuffer(
                input_mem_buffer, convert_to_bands(tile["data"], mode).read()
            )

            if (
                gdal_file_info.init_from_name(input_mem_buffer) == 1
            ):  # add else for error
        ...
        if t_srs != "EPSG:3857":
            ...
            reproject_image(
                gdalwarp_bin, result_filename, "EPSG:3857", reproject_filename, t_srs
            )
        ...
        if t_format != "GTiff":
            ...
            convert_image(
                gdal_translate_bin, result_filename, convert_filename, t_format
            )
            move(
                splitext(convert_filename)[0] + ".wld",
                splitext(convert_filename)[0] + ".jgw",
            )
            # os.remove(result_filename)
            log.info("Removed %s" % result_filename)
...
def convert_image(bin, src, dest, format="JPEG"):
    p = Popen(
        ["gdal_translate", "-of", format, "-co", "worldfile=yes", src, dest],
        stdout=PIPE,
        stderr=STDOUT,
    )
    if p.wait() != 0:
...
def move_files(src, dest):
    ...
    for file_name in glob.glob(join(dirname(src), src_basename + ".*")):
        ...
        dest = join(
            dest_dirname, basename(file_name).replace(src_basename, dest_basename)
        )
