...
class ContextModelUpdater(object):
    ...
    def source_from_layer(self, layer, first):
        source = (
            self.session.query(model.ExternalWMTSSource)
            .filter_by(name=layer["name"])
            .all()
        )
        if source:
            source = source[0]
        ...
        if "view_restriction" in layer:
            source.view_coverage = self.coverage_from_restriction(
                layer["view_restriction"]
            )
        ...
        if "download_restriction" in layer:
            source.download_coverage = self.coverage_from_restriction(
                layer["download_restriction"]
            )
            source.download_level_start = layer["download_restriction"].get(
                "zoom_level_start"
            )
            source.download_level_end = layer["download_restriction"].get(
                "zoom_level_end"
            )
        else:
...
def reload_context_document(app_state, user, password):
    ...
    result = requests.get(
        app_state.config.get("web", "context_document_url"), auth=(user, password)
    )
    ...
    all_active_sources = set(
        session.query(model.ExternalWMTSSource).filter_by(active=True).all()
    )
    ...
    couchdb = CouchDB(
        "http://127.0.0.1:%d" % app_state.config.get_int("couchdb", "port"),
        "_replicator",
    )
